...
Built through PRD §14 **T1–T6**, plus a security migration (`0007`). Not yet built: **T7** (Deepgram audio/video) and **T8** (tag approve/merge, audit-log writes, orphaned-storage cleanup in the cron sweep).
...
npm run db:start     # local Supabase in Docker (no API keys needed)
npm run db:reset     # apply supabase/migrations/0001..0005 from scratch
npm run types:db     # regenerate src/lib/supabase/database.types.ts
npm run seed:admin   # idempotent; asserts has_claim(...) === true
```
